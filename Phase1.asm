...
lower_initial_point db '$$$';(m.s)assume max initial point is 99 in deciaml=>dose not acceed 8 bits
;
...
reg_AX_V_P_zeroes db "0000$"
reg_BX_V_P_zeroes db "0000$"
reg_CX_V_P_zeroes db "0000$"
reg_DX_V_P_zeroes db "0000$" 
reg_SI_V_P_zeroes db "0000$"
reg_DI_V_P_zeroes db "0000$"
reg_SP_V_P_zeroes db "0000$"
reg_BP_V_P_zeroes db "0000$" 
...
call Print_MSG  
mov dx,0100h 
mov di,offset lower_initial_point
call Print_MSG  
mov dx,0014h 
mov di,offset User2_Name+2
call Print_MSG   
...
Users_screen             PROC  
   ...
   push_result:   ; mov ax,offset lower_initial_point 
                  ; mov [ax],dx 
                   ...
                   div bl;al=ax/bl
                   add ah,30h
                   mov lower_initial_point,ah
                   add al,30h
                   mov di ,offset lower_initial_point+1
                   mov [di],al;as we assume the max initial point is 99 in deciaml
...
Player1:
        ...
        second_operand: 
            ...
            mov al, byte ptr offset exepression + 1
        ...
        not_valid:
        ; push es
        ; ;clear then go to third_special case again
        ; mov cx,24 ;Column
        ; mov dx,176 ;184-8(no +1 here as above)
        ; mov al,3 ;Pixel color
        ; mov bx,320 ;width=320 
        ...
        ; call draw_rectangle
        ; pop es
        ; jmp _finish_selection
        ;then clear
        ;then prompt for the user ot input again

        ;check if it is a special case ---> inc dec
        if_third_special_case:
            mov bx, offset selected_command
            mov al, [bx]
            cmp al, 'i'
            jne if_fourth_special_case
            push es
            ;clear then go to third_special case again
            mov cx,24 ;Column
            mov dx,176 ;184-8(no +1 here as above)
            ...
            call draw_rectangle
            pop es
            jmp third_special_case



        if_fourth_special_case:
            mov bx, offset selected_command
            mov al, [bx]
            cmp al, 'd'
            jne read_exepression_again
            ;clear then go to third_special case again
            push es
            mov cx,24 ;Column
            mov dx,176 ;184-8(no +1 here as above)
            ...
            call draw_rectangle
            pop es
            jmp fourth_special_case


        read_exepression_again:
            ;clear then go to third_special case again
            push es
            mov cx,24 ;Column
            mov dx,176 ;184-8(no +1 here as above)
...
;let me have the charachter
        
         
        ;read 1st powerup character
        ;we get ascii
        push ax
        mov ah,0
        int 16h
        mov Power_char,al
        pop ax
        ;1st powerup 1 code be as following 
        cmp Power_char,'+'
        jz pow1
        cmp Power_char,'-'
        jz pow2
        ...
        pow1:
        ;decrement P1 points here by 5
        mov _player_proc,1 
        call  ExecuteCommand
        jmp clear_what_player1_typed
        pow2:
        ;decrement P1 points here by 3
        push si
        push di
        push cx
        mov si,offset operand_A
        mov di,offset operand_A_buffer
        mov cx,6
        rep movsb
        mov si,offset operand_B
        mov di,offset operand_B_buffer
        mov cx,7
        rep movsb
        pop cx
        pop di
        ...
        push di
        push cx
        mov si,offset operand_A_buffer
        mov di,offset operand_A
        mov cx,6
        rep movsb
        mov si,offset operand_B_buffer
        mov di,offset operand_B
        mov cx,7
        rep movsb
        ...
        CMP Pow3usedBefore,1
        JZ clear_what_player1_typed
        ...
        pop dx
        pop ax
        mov Pow3usedBefore,1
        JZ clear_what_player1_typed


        pow5:
        ;TO TEST COMMAND ONCE ON OUR REG
        ; mov _player_proc,1 
        ; call  ExecuteCommand
        CMP Pow5usedBefore,1
        JZ clear_what_player1_typed

        push cx
        push dx
        MOV SI,OFFSET reg_AX_V_P_zeroes
        mov di,offset reg_AX_V_P1
        MOV CX,40
        REP MOVSB
...
; ;---
        ...
        ;decrement P1 points here by 30
        ; mov reg_AX_V_P1,"0000$"
        ; mov reg_BX_V_P1,"0000$"
        ; mov reg_CX_V_P1,"0000$"
        ; mov reg_DX_V_P1,"0000$"
        ; mov reg_DI_V_P1,"0000$"
        ; mov reg_SI_V_P1,"0000$"
        ; mov reg_BP_V_P1,"0000$"
        ; mov reg_SP_V_P1,"0000$"
        ; mov Pow5usedBefore,1

        ;jmp outer loop        

        jmp clear_what_player1_typed ; IN CASE I MADE MORE POWERUPS BELOW
...
Player2:
        ...
        _select_command2:
            ...
            ; int 21h
            jne go2
            jmp far ptr _finish_selection2
        ...
        first_special_case2:
            mov al, [bx]
            cmp al, 'n'
            jne second_special_case2
            mov operand_A, ' '
            mov operand_B, ' '
            jmp end_of_validation2


        ;if clc  --> no operand
        second_special_case2:
            mov al, [bx]
            cmp al, 'c'
            jne third_special_case2
            mov operand_A, ' '
            mov operand_B, ' '
            jmp end_of_validation2
        ...
        third_special_case2:
            ...
            ;validate
            dec size_a
            jmp validate_first_operand2




        ;if dec  --> 1st operand --> musn't be value
        fourth_special_case2:
            mov al, [bx]
            cmp al, 'd'
            jne read_exepression2
            mov operand_B, ' '
            ;read the first operand and validate it
        ...
        validate_first_operand2:    ;first operand can't be value
            ...
            check_reg_or_offset2:
                ...
                check_12:
                    ...
                    next_1_12:
                        ...
                        je _8_next2 
                        mov bx, offset operand_A + 1
                        jmp far ptr check1_char_1_frist_option2
                        _8_next2:
                        ;remove H
                        mov byte ptr [bx], ']'
                        inc bx 
                ...
                check_22:
                    ...
                    _12_next2:
                    mov bx, offset operand_A + 1 ; +1 to skip the [
                    check_0_to_22:    ;[value 0->f/F]   [0->9]
                        mov al, [bx]
                        mov cx,22   ;22 --> the size of array of valid_values_memo 
                        mov di, offset valid_values_memo
                        REPNE SCASB
                        cmp cx, 0
                        jg go_to_validate_second_opernad2
                        cmp al,'F'
                        je _9_next2
                        jmp far ptr not_valid2
                        _9_next2:
                        jmp validate_second_operand2



        jmp go_next_next2
        go_to_validate_second_opernad2:
        jmp far ptr validate_second_operand2
        go_next_next2:
                ...
                check_32:
                    ...
                    check1_char_1_frist_option2:
                        ...
                        jne check1_char_2_first_option2
                        cmp al, 'c'
                        je check1_char_2_first_option2
                        dec bx
                        jmp check1_char_1_second_option2  ; i know that no need to write this jmp as it is the next line --> but it is for me to notice temp. !!!!!!!!!!!!!!!!!!!!





                    check1_char_1_second_option2:   
                        mov di, offset first_char_reg_option_2
                        mov al, [bx]
                        mov cx,2  ; 2 --> size of the array above
                        REPNE scasb 
                        ...
                        jne check1_char_2_second_option2
                        cmp al, 's'
                        je check1_char_2_second_option2
                        dec bx
                        jmp check1_char_1_third_option2
                



                    check1_char_1_third_option2:
                        mov di, offset first_char_reg_option_3
                        mov al, [bx]
                        mov cx,2  ; 2 --> size of the array above
                        REPNE scasb
                        ...
                        jne check1_char_2_third_option2
                        cmp al, 'b'
                        je check1_char_2_third_option2 
                        dec bx
                        jmp check1_char_1_fourth_option2
                    ...
                    check1_char_1_fourth_option2: 
                        ...
                        jne check1_char_2_fourth_option2
                        cmp al, 'd'
                        je check1_char_2_fourth_option2 
                        dec bx
                        jmp not_valid2
        ...
        ;;;;;;;;;;;;;;;;;;;;;;;;;;
                    ...
                    check1_char_2_first_option2: 
                        ...
                        REPNE scasb
                        cmp cx,0
                        jne validate_second_operand2
                        cmp al,'x'
                        je validate_second_operand2 
                        jmp not_valid2
        ...
        validate_second_operand2:
            ...
            cmp al, 0
            jne go_here_2
            jmp far ptr end_of_validation2
            go_here_2:
            check_if_value2:     ; 0 --> FFFF  --->  0FFFFH    FFFFH,FFFH,    0FFFF
                mov bx, offset operand_B 
                mov cl, size_b
                cmp cl, 6
                jg go_to_not_valid_2_22
                cmp cl,6
                jl loop_validate_each_char2  
                ;here the size is equal to 6
                mov bx, offset operand_B + 5
                cmp byte ptr [bx], 'h'
                je  prepare_to_looperand_B
                cmp byte ptr [bx], 'H'
                jne go_to_not_valid_2_22
                prepare_to_looperand_B:
                    ;remove h
                    mov byte ptr [bx], '$'
                    ;dec the size
                    dec size_b
                ...
                loop_validate_each_char2:
                    ...
                    ;check that the first char is 0 or last char is h otherwise --> it is not value --> offset
                    cmp byte ptr [bx], '0' 
                    jne check_if_last_is_h2
                    inc bx   ; to go to the next char 
                    dec cl   ; for the loop
                    mov bx,offset operand_B + 1
                    jmp next_looperand_B  
                    ...
                    check_if_last_is_h2: 
                        mov bx, offset operand_B + 4
                        cmp byte ptr [bx], 'h'
                        je  _prepare_to_looperand_B
                        cmp byte ptr [bx], 'H'
                        je _prepare_to_looperand_B
                        jmp far ptr check_if_reg_or_offset2  ;if neither not ---> can be offset
                        _prepare_to_looperand_B:
                        ;remove h
                        mov byte ptr [bx], '$'
                        ;dec the size
                        dec size_b
        ...
        next_here_2__2:  
                ...
                inc cl
                    next_iteration2:
                    cmp byte ptr [bx],'0'
                    jl check_if_reg_or_offset2
                    cmp byte ptr [bx],3Ah  ;':'  --> the ascii of the char after 9 
                    jl end_iteration2;is a number

                    cmp byte ptr [bx],'A'
                    jl check_if_reg_or_offset2
                    cmp byte ptr [bx],'G'
                    jl end_iteration2 ; is a cap letter

                    cmp byte ptr [bx],'a'
                    jl check_if_reg_or_offset2
                    cmp byte ptr [bx],'g'
                    jl end_iteration2; is a small letter
                    
                    jmp check_if_reg_or_offset2
                ...
                cmp byte ptr [bx],3Ah  ;':'  --> the ascii of the char after 9 
                jnl here_22
                jmp far ptr end_of_validation2;is a number
                ...
                cmp byte ptr [bx],'G'
                jnl here_32
                jmp far ptr end_of_validation2 ; is a cap letter
                ...
                cmp byte ptr [bx],'g'
                jnl if_not_number2
                jmp far ptr end_of_validation2; is a small letter
        
                
                
                
                if_not_number2:   ; if the last char is not a number so it must be h
                cmp byte ptr [bx], 'h'
                je  here_come2
                cmp byte ptr [bx], 'H'
                jne check_if_reg_or_offset2               ;modified
                here_come2:
                ;remove h
                mov byte ptr [bx], '$'
                ;dec the size
        ...
        next_here_12:
                ...
                _check_12:
                    ...
                    _next_1_12:
                        ...
                        je _8_next_2
                        mov bx, offset operand_B + 1
                        jmp far ptr _check1_char_1_frist_option2
                        _8_next_2:
                        ;remove H
                        mov byte ptr [bx], ']'
                        inc bx 
        ...
        next_here_02:         
                ...
                _check_22:
                    ...
                    _14_next2:
                    mov bx, offset operand_B + 1 ; +1 to skip the [
                    _check_0_to_22:    ;[value 0->f/F]   [0->9]
                        mov al, [bx]
                        mov cx,22   ;22 --> the size of array of valid_values_memo 
                        mov di, offset valid_values_memo
                        REPNE SCASB
                        cmp cx, 0
                        jg go_to_end_of_validation2
                        cmp al,'F'
                        jne go_to_not_valid2
                        jmp end_of_validation2

        jmp next_go2
        go_to_end_of_validation2:
        jmp far ptr end_of_validation2
        next_go2:
                ...
                _check_32:
                    ...
                    _check1_char_1_frist_option2:
                        ...
                        inc bx
                        CMP CX,0   ;if reach 0 ---> 2 options --> either = last element    /or   doesn't exist 
                        jne _check1_char_2_first_option2
                        cmp al, 'c'
                        je _check1_char_2_first_option2 
                        dec bx
                        jmp _check1_char_1_second_option2  ; i know that no need to write this jmp as it is the next line --> but it is for me to notice temp. !!!!!!!!!!!!!!!!!!!!





                    _check1_char_1_second_option2:   
                        mov di, offset first_char_reg_option_2
                        mov al, [bx]
                        mov cx,2  ; 2 --> size of the array above
                        REPNE scasb
                        inc bx
                        CMP CX,0   ;if reach 0 ---> 2 options --> either = last element    /or   doesn't exist 
                        jne _check1_char_2_second_option2
                        cmp al, 's'
                        je _check1_char_2_second_option2
                        dec bx
                        jmp _check1_char_1_third_option2
                



                    _check1_char_1_third_option2:
                        mov di, offset first_char_reg_option_3
                        mov al, [bx]
                        mov cx,2  ; 2 --> size of the array above
                        REPNE scasb
                        inc bx
                        CMP CX,0   ;if reach 0 ---> 2 options --> either = last element    /or   doesn't exist 
                        jne _check1_char_2_third_option2
                        cmp al, 'b'
                        je _check1_char_2_third_option2  
                        dec bx
                        jmp _check1_char_1_fourth_option2
        ...
        next_here2:
                    ...
                    _check1_char_1_fourth_option2:
                        ...
                        inc bx
                        CMP CX,0   ;if reach 0 ---> 2 options --> either = last element    /or   doesn't exist 
                        jne _check1_char_2_fourth_option2
                        cmp al, 'd'
                        je _check1_char_2_fourth_option2
                        dec bx
                        jmp not_valid2
        ...
        ;;;;;;;;;;;;;;;;;;;;;;;;;;
                    ...
                    _check1_char_2_fourth_option2:
                        mov di, offset second_char_reg_option_4
                        mov al, [BX]  
                        mov cx,8
                        REPNE scasb
                        cmp cx,0
                        jne end_of_validation2
                        cmp al,'x'
                        je end_of_validation2 
                        jmp not_valid2
        ...
        ;check if it is a special case ---> inc dec
        if_third_special_case2:
            mov bx, offset selected_command
            mov al, [bx]
            cmp al, 'i'
            jne if_fourth_special_case2
            push es
            ;clear then go to third_special case again
            mov cx,24 ;Column
            mov dx,176 ;184-8(no +1 here as above)
            ...
            call draw_rectangle
            pop es
            jmp third_special_case2



        if_fourth_special_case2:
            mov bx, offset selected_command
            mov al, [bx]
            cmp al, 'd'
            jne read_exepression_again2
            ;clear then go to third_special case again
            push es
            mov cx,24 ;Column
            mov dx,176 ;184-8(no +1 here as above)
            ...
            call draw_rectangle
            pop es
            jmp fourth_special_case2


        read_exepression_again2:
            ;clear then go to third_special case again
            push es
            mov cx,24 ;Column
            mov dx,176 ;184-8(no +1 here as above)
